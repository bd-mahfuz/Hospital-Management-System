	...
	<div class="container">
		<div>
			<div class="tab">
			  ...
			  <button class="tablinks" onclick="openCity(event, 'Paris')"><i class="fa fa-lock"></i>
			...
			<div id="London" class="tabcontent">
			  <div class="adddep">
			  	<form id="form1" action="../doctor/editDProfile.jsp" method="post">
			  		<%
			  		String mail=session.getAttribute("userMail").toString();
			  		List<AddDoctor> list=DoctorDao.getRecordsById(mail);
			  		String name="";
			  		String email="";
			  		String address="";
			  		String phone="";
					for(AddDoctor an:list) {
						name=an.getDoctorName();
						email=an.getDoctorEmail();
						address=an.getAddress();
						phone=an.getPhone();
					}
			  		%>
			  		<div>
			  			<input type="hidden" name="id" value=""/>
			  			<label>Name :</label><input type="text" name="name" value="<%=name %>" placeholder="Name" />
			  		</div>
			  		<div>
			  			<label>Email :</label><input type="text" name="email" value="<%=email %>" placeholder="Email" />
			  		</div>
			  		<div>
			  			<label>Address :</label><input type="text" name="address" value="<%=address %>" placeholder="Address" />
			  		</div>
			  		<div>
			  			<label>phone :</label><input type="text" name="phone" value="<%=phone %>" placeholder="Phone Number" />
			  		</div>
			  		<div class="depbtn">
				  		<div class="btn-group">
			...
			<div id="Paris" class="tabcontent">
			  <div class="adddep">
			  	<form id="form" action="../doctor/changeDPassword.jsp" method="post">
			  		
			  		<div>
			  			<input type="password" name="ppass" value="" placeholder="Previous password"/>
			  		</div>
			  		<div>
			  			<input type="password" name="npass" value="" placeholder="New password"/>
			  		</div>
			  		
			  		
				  	<div class="depbtn">
				  		<div class="btn-group">
						  <a href="javascript:;" onclick="document.getElementById('form').submit();" class="btn btn-primary">Change Password</a>
						  <input type="hidden" name="mess" value="Save"/>
						</div>	
						<div class="btn-group">
						  <button class="btn btn-primary" type="reset" cursor="hand">Reset</button>
