...
/**
 * Servlet implementation class DocAddPatientInfo
 */
@WebServlet("/DocAddPatientInfo")
public class DocAddPatientInfo extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public DocAddPatientInfo() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (i>0) {
				request.setAttribute("message", "Pateint info inserted");
				request.getRequestDispatcher("doctor/addPatient.jsp").forward(request, response);
			}else{
				request.setAttribute("message", "Pateint info not inserted");
				request.getRequestDispatcher("doctor/addPatient.jsp").forward(request, response);
			}
		} catch (Exception e) {
			e.printStackTrace();
